...
3. Launch one agent per task in the wave: each in its own git worktree on its own branch (mechanics in [docs/07-Git-Workflow.md](../docs/07-Git-Workflow.md)), each following the implementation prompt ([implementation.md](implementation.md)) with its Wave/Branch assignment recorded on the task. Each agent runs on its role's default model — the tier installed in its agent definition per [docs/17-Model-Tiers.md](../docs/17-Model-Tiers.md). You may apply a dispatch-time **upgrade-only** override — BUILD → REASONING for one unusually complex task — recording the override and its reason in the task's Notes. Never downgrade: no agent runs below its role's tier, and gate roles never leave REASONING.
...
5. **Record what you decided, not what the agents did** ([docs/18-Board.md](../docs/18-Board.md)). Yours: the wave and branch assignment at dispatch (`node board/cli.mjs dispatch <id> --wave <wave> --branch <branch>`), `DONE` once the branch is merged, and a written blocker on any member you park (`node board/cli.mjs block <id> "<blocker>" --actor agent:orchestrator`). The wave agents record their own IN_PROGRESS and REVIEW — do not write those for them, and do not re-write a state an agent already recorded.
