...
5. Update task states as tasks move ([docs/10-Task-Lifecycle.md](../docs/10-Task-Lifecycle.md)) and record wave assignments (the task **Wave** field) so the dispatch is auditable — `node board/cli.mjs state <id> <STATE> --actor agent:orchestrator --wave <wave> --branch <branch>` writes the task file and the board history in one step ([docs/18-Board.md](../docs/18-Board.md)); park a failed member with `node board/cli.mjs block <id> "<blocker>"`.
