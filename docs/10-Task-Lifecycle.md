...
- **Every transition is recorded.** Moving a task means editing its State field *and* leaving a line in the board's history ([17-Board.md](17-Board.md)) — one command does both: `node board/cli.mjs state <id> <STATE> --actor agent:<role>`. A transition nobody can reconstruct afterwards did not happen.
