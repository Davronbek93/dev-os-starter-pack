...
The board ([18-Board.md](18-Board.md)) is a view over this model, not an exception to it: it displays waves, readiness, and Touches overlap, and it queues requests — but every dispatch decision is still made by the orchestrator, in one place, when a human starts a session.
